...
def shortest_path(source, target):
    ...
    """
    # solution heavily based on code used in lecture to solve the maze puzzle
    # solution usilizes Queue Frontier (breadth first search)

    # return none if the source and target is the same person
    if source == target:
        return None
    ...
    while True:
        ...
        for state in neighbors_for_person(node.state[1]):
            if not frontier.contains_state(state) and state[1] not in explored:
                ...
                # If child is the goal, then we have a solution, return it
                ...
                # add to frontier if it's not the goal
                frontier.add(child)
